...
\textit{Mechanism}  &    \multicolumn{2}{|c|}{Price} &  \multicolumn{4}{c|}{Trust} &  \multicolumn{1}{c}{Risks} \\
...
%========================%
%=====Gas plot justification====%
%Compared to Figure~\ref{fig:fiatandcrypto} and Figure~\ref{fig:all} (see Section~\ref{sec:plots}), Figure~\ref{fig:gas} has mostly diagonal changes, spread over a smaller range. Thus, gas is relatively stable compared to Ether. Also, it is only changing over a small scale in x-axis (USD), when compared to Ether's plot over the same axis in a larger interval. Note that the only two spikes in the plot represent two specific events occurred on the Ethereum blockchain on those certain dates. 
% Need some explanation for Gas, ether, and electricity
