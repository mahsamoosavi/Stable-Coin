...
%The idea seigniorage shares was first introduced by Robert Sams in 2017, where he proposed a new dual token system that uses two coins to achieve price stability: (i) a stablecoin (\eg fiat currency) and (ii) a volatile coin (\eg equity shares)~\cite{sams2015note}.
 ...
 %Bitmex provides a classification for the stablecoins based on the fact whether there is an underlying collateral or not. They focus on the distributed stablecoins. We take centralized projects into consideration too.
% Bitmax  the two types of stablecoin and provided a comparison in terms of different design decisions they apply~\cite{Bitmex}.
...
\paragraph{Process.} Alice uses Ethereum to generate a contract to issue 1000 AliceCoins as standard (\eg ERC20) tokens. She lists asks \$1 USD for 1 AliceCoin and promises to redeem the tokens for \$1 USD. If Bob buys 10 AliceCoins for \$10 USD, Alice deposits the \$10 USD in a bank account. Any time Alice receives a request for AliceCoins but does not have any left to sell, she creates new ones. If Carol wants to redeem 5 AliceCoins, Alice withdraws \$5 USD and exchanges it with Carol and takes the AliceCoins out of circulation. Alice frequently obtains bank statements showing that her account holds USD equivalent to the number of coins in circulation (the number of AliceCoins can be checked anytime on Ethereum).

\paragraph{Price Stability.} For simplicity, assume all transactions are free and frictionless. Consider a bid for 1 AliceCoin that is greater than \$1 USD. Alice will generate a new AliceCoin, deposit only \$1 USD, and keep the rest of the bid value as profit. Therefore bids in excess of \$1 USD will be temporal. Next, consider an ask for 1 AliceCoin for less than \$1 USD. Anyone can purchase this AliceCoin for its asking price, try to redeem it for \$1 USD from Alice, and if successful, keep the difference as profit. Therefore the ask price of an AliceCoin is proportional to the expectation that it can actually be redeemed for \$1 USD. For an example, an ask of \$ 0.60 USD reveals that the trader believes there is only a 60\% chance of redeeming the coin (otherwise he'd redeem it rather than sell it). 

 Conclusion: the bid-ask spread will saddle \$1 USD and trades should execute close to \$1 USD. 

\paragraph{Discussion.} 

% Discussion: transactions are decentralized but creation, reserves, redemption are not, still strictly better. 
% Discussion: role of audit
