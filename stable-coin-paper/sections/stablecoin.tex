...
% In Ethereum blockchain, every transaction contains a number of operations and there is a precise amount of gas unit associated with each.\footnote{http://ethdocs.org/en/latest/contracts-and-transactions/account-types-gas-and-transactions.html\#what-is-gas} Since Ether is volatile, this concept is introduced to have fixed cost for operations.

% Thus, even though the price of Ether changes, the amount of Ether corresponding to a unit of gas decreases; so that users pay the same price for the transactions on the Ethereum blockchain. Hence, the notion of gas is yet to be investigated to find out whether it exhibits behavior similar to fiat currencies or cryptocurrencies.
...
% Important note: If some companies (for example Fcoin) knows that they're gonna be sending lots of txs to the network that will definitely increase the Gas price in the future, they can store this gastoken ahead of time and benefit from using cheap Gas when the price is up.
...
%========================%
%=====Gas plots justification====%

%BTC and Ether are the kind of the same, Elect and USD are kind of the same
%Those two spikes in the gas plot are not because the change in the price of BTC, Ether, Elects. As mentioned, those changes are related to the specific events on the Etherum blockchain that increase the gas price on certain dates. In other words, the price of gas itself changed without being related to anything.
% Everytime we plot gas with respect to anything we’ll have those spikes anyways, again because these changes are in the gas price itself.
