...
\subsection{Blockchain metrics input; with only internal information}
% Description of how it works.
% Justification that bids will never exceed $1
% Justification that offers will be less than $1
% Examples:
...
\hline
	...
	\textit{Mechanism}  &    \multicolumn{2}{|c|}{Price} &  \multicolumn{4}{c|}{Trust}  \\
	...
	Market Manipulation                       	& \multicolumn{1}{|c|}{?} 		& \multicolumn{1}{c|}{\full}   & \multicolumn{1}{c|}{\full} 	& \multicolumn{1}{c|}{\~} 		& \multicolumn{1}{c|}{\full} & \multicolumn{1}{c|}{}    \\
	Money Supply Adjustments             	& \multicolumn{1}{|c|}{?} 		& \multicolumn{1}{c|}{\full}   & \multicolumn{1}{c|}{\full} 	& \multicolumn{1}{c|}{\~} 		& \multicolumn{1}{c|}{\full} & \multicolumn{1}{c|}{}    \\
	Asset Transfer	                                & \multicolumn{1}{|c|}{?} 		& \multicolumn{1}{c|}{\full}   & \multicolumn{1}{c|}{\full} 	& \multicolumn{1}{c|}{\~} 		& \multicolumn{1}{c|}{\full} & \multicolumn{1}{c|}{}    \\
	\hline

	\end{tabular}
	\caption{Comparative evaluation of mechanisms to design stablecoins. {\full} indicates the properties (columns) are fulfilled by the corresponding mechanisms (rows). {\prt} partially fulfilled. (\textblue{is there any other newly defined sign?})
...
%GRAPHS:
%Both in Figure~\ref{fig:fiatandcrypto} and Figure~\ref{fig:all}, the currencies are plotted against two reference currencies: a cryptocurrency, BTC, on y-axis and a fiat currency, USD, on x-axis to capture the movement in their prices against these two different group of currencies. Figure~\ref{fig:fiatandcrypto} illustrates the volatility in cryptocurrencies. Both ETH and XRP exhibit movements mostly in 4/8 and 2/6 directions. Also, the fact that the points are spread in a large range of values indicates drastic changes in ETH price with respect to BTC and USD. Here, the main question to bear in mind is whether ETH and XRP move with BTC. The diagonal movement of the plotted values suggests that they do not move along with BTC.

%Compared to the previous figure, Figure~\ref{fig:all} shows stable currencies. CAD and EUR are chosen to show the behavior in two different currency zones. For the stablecoins, Tether and BitUSD are plotted. Both the government-issued currencies and the stablecoins mostly move along the 1/5 direction, showing that a stable currency moves along a vertical line with mild fluctuations. It is important to note that these fluctuations are accentuated due to the scale. For instance, it seems like Tether is undergoing drastic changes in price. However, the price change is around 10 cents.
...
In this paper, we provide a survey of active stablecoin projects. Unlike previous research studies performed on this topic, we selectively use the concepts from finance while eliminating the jargons. In addition, rather than focusing on the details of how particular `brands' of stablecoins work, we thoroughly describe the fundamental mechanics and concepts to achieve price stability. Respectively, we show the taxonomy we use to classify stablecoins which represents the techniques to build a stablecoin. Eventually, we explore the potential stable index-cryptocurrencies (namely Ethereum gas) in the context of stablecoins.
